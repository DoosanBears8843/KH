...
def title(msg):
    ...
    chromedriver_dir = r"C:\Users\User\Desktop\Mbot\chromedriver.exe"
...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("야구장에서 응원"))
...
async def 들어와(ctx):
    try:
        ...
        try:
            ...
            await ctx.send("채널에 유저가 접속해있지 않다.")
...
async def 나가(ctx):
    try:
        ...
        await ctx.send("이미 그 채널에 속해있지 않다.")
